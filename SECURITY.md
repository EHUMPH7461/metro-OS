...
Phase B upgrades Electron to `43.2.0`. The validated branch reports zero vulnerabilities from `npm audit`. Context isolation, renderer sandboxing, disabled Node integration, the narrow preload bridge, main-process IPC validation, CSP, unchanged user-data isolation, and main-process-only credential access remain enforced. This is a maintenance candidate, not a released security update, until reviewed and merged.
